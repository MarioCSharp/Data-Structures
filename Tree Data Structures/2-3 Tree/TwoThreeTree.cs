namespace _01.Two_Three
{
    ...
    public class TwoThreeTree<T> where T : IComparable<T>
    {
        ...
        private TreeNode<T> Insert(TreeNode<T> node, T element)
        {
            ...
            if (node.IsLeaf())
            {
                return this.Merge(node, new TreeNode<T>(element));
            }

            if (this.IsLesser(element, node.LeftKey))
            {
                var newNode = this.Insert(node.LeftChild, element);

                return newNode == node.LeftChild ? node : this.Merge(node, newNode);
            }
            else if (node.IsTwoNode() || this.IsLesser(element, node.RightKey))
            {
                var newNode = this.Insert(node.MiddleChild, element);

                return newNode == node.MiddleChild ? node : this.Merge(node, newNode);
            }
            else
            {
                var newNode = this.Insert(node.RightChild, element);

                return newNode == node.RightChild ? node : this.Merge(node, newNode);
            }
        }
        ...
        private TreeNode<T> Merge(TreeNode<T> currentNode, TreeNode<T> node)
        {
